...
function cio_set_default_score($id) {
  
  if (!GetVotes($id)) {
    // set user meta so we can query against it.
		update_post_meta( $id, 'score', 0);
  }
}
add_action('save_post', 'cio_set_default_score');
...
function cio_display_feed($feed_url, $count = 100, $query_text, $source_name, $source_url) {
        ...
        <p>searching "<a href="<?php echo $source_url; ?>"><?php echo $query_text; ?></a>" <small><?php edit_post_link('edit query', ' (', ')'); ?></small>
        </p>
