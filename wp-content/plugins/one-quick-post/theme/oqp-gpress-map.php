...
//Duplicated from gpress/gpress-core/tinymce/map.php with some differences
...
global $oqp_gpress_marker_key;
$marker_key=(int)$oqp_gpress_marker_key;


$marker = $oqp_extensions['geo']->get_post_position();
...
function initialize<?php echo $map_id; ?>() {
	...
	$marker = $oqp_form->loaded_post->geo->marker;

	
	if ($marker['position']) {
	?>
		var latLng = new google.maps.LatLng(<?php echo $marker['position'][0];?>,<?php echo $marker['position'][1];?>);
	<?php
	}else {
	?>
...
<div style="clear:both; float:left; width:100%; margin-bottom:15px;">  
	...
	<div style="float:left; margin-bottom:20px; width:100%;" class="other_stuff<?php echo $map_id; ?> gpress_otherstuff">
		<input id="search_address<?php echo $map_id; ?>" name="<?php echo oqp_get_form_field_input_name();?>[<?php echo $marker_key;?>][search]" type="textbox" value="<?php echo $marker['search'];?>" class="search_input">
		<input type="button" value="<?php _e('Search');?>" onclick="codeAddress<?php echo $map_id; ?>();" class="search_button">
		<input type="button" class="oqp-gpress-clear" value="<?php _e('Clear');?>" onClick="oqp_gpress_removeTag(this)"/>
		<label for="marker-title">Marker Title:</label>
		<input type="text" id="marker-title" class="gpress" name="<?php echo oqp_get_form_field_input_name();?>[<?php echo $marker_key;?>][title]" value="<?php echo $marker['title'];?>"/>
	...
	<div class="other_stuff<?php echo $map_id; ?> gpress_otherstuff" style="display:none">
		<input id="map-position" class="gpress_locationvalue gpress" type="textbox" name="<?php echo oqp_get_form_field_input_name();?>[<?php echo $marker_key;?>][position]" style="width:100%; float:left; margin:25px 0 10px;">
	</div>
...
<div class="gmaps_meta_boxes other_stuff<?php echo $map_id; ?> gpress_otherstuff" style="display:none !important;">
    <div id="infoPanel<?php echo $map_id; ?>" class="gpress_infopanel">
        <div id="leftColumn<?php echo $map_id; ?>" class="gpress_leftcolumn">
            <b>Closest address:</b>
            <textarea id="address<?php echo $map_id; ?>" class="gpress_address" name="<?php echo oqp_get_form_field_input_name();?>[<?php echo $marker_key;?>][address]" value="<?php echo $marker['address'];?>" class="closest_address"></textarea>
        </div>
